defmodule Phoenix.Endpoint.ChatterboxHandler do
  @moduledoc """
  ...
  *NOTE*: This feature depends on the internals of Cowboy 1.0 API
  ...
  def child_spec(scheme, endpoint, config) do
    ...
    dispatches =
      for {path, socket} <- endpoint.__sockets__,
          ...
          handler = config[:chatterbox] || default_for(module),
          do: {Path.join(path, Atom.to_string(transport)),
               handler,
               {module, {endpoint, socket, transport}}}

    dispatches =
      dispatches ++ [{:_, Plug.Adapters.Chatterbox.Handler, {endpoint, []}}]
    ...
    {ref, mfa, type, timeout, kind, modules} =
      Plug.Adapters.Chatterbox.child_spec(scheme, endpoint, [], config)
  ...
  defp default_for(Phoenix.Transports.LongPoll), do: Plug.Adapters.Chatterbox.Handler
  defp default_for(_), do: nil

  @doc """
  Callback to start the Chatterbox endpoint.
  """
  ...
  defp info(scheme, endpoint, ref) do
    ...
    "Running #{inspect endpoint} with Chatterbox using #{scheme}://localhost:#{port}"
  end
end
